...
function change (data, callback) {
    ...
    log.info('[' + data.seq + '/' + latestSeq + '] starting to process', json.name);
...
function setTimer () {
    function timer() {
        ...
        log.info('fetching https://skimdb.npmjs.com/registry');
        http.get(u, function(res) {
            log.info('got ' + res.statusCode + ' from https://skimdb.npmjs.com/registry');
            var d = '';
            ...
            res.on('end', function() {
                ...
                if (json.update_seq) {
                    ...
                    log.warn('failed to fetch udpate_seq from https://skimdb.npmjs.com/registry');
                    log.warn(d);
                }
            });
        });
